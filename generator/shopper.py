...
# Assume all payments are made with credit card
# There name and address are recorded but credit card number is not recorded for simplicity

id = 1
paymentid = 340000
# Average # of fans per game is 18000
# Assume 1/4 fans is a shopper
# 4500 shoppers per game
# 82 reg + 8 pre max + 16 post max = 96 games
# 96 * 4500 * 50 seasons = 21M shoppers
...
with open('data/shopper.csv', mode='w') as shopper_file:
    shopper_writer = csv.writer(shopper_file, delimiter=',', quotechar='"', quoting=csv.QUOTE_MINIMAL)

    # id, paymentid, first_name, last_name, full_name, street address, zip
    while paymentid < 5000000:
        shopper = []

        shopper.append(id)

        shopper.append(paymentid)

        # Generage a random gender
        gender = randint(0, 1)

        # First name
        shopper.append(names.get_first_name(gender = 'male')) if gender == 1 \
            else shopper.append(names.get_first_name(gender = 'female'))

        # Last name
        shopper.append(names.get_last_name())

        # Full Name
        shopper.append(shopper[2] + ' ' + shopper[3])
        ...
        # Address
        shopper.append(str(addNum) + " " + streetPrefix[addPre].title() + " " + streetSuffix[addSuf])

        # Zip
        zip = randint(10000,100000)
        shopper.append(zip)

        paymentid += 1
        id += 1

        shopper_writer.writerow(shopper)
